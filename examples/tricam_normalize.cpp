...
int tricam_process(int argc, char *argv[])
{
  ...
  point_t *centroid;
  int label, area;
  uint8_t up_min, mid_min, dn_min;
  uint8_t up_max, mid_max, dn_max;
  real_t up_mean, mid_mean, dn_mean;
  real_t up_var, mid_var, dn_var;
  ...
  label_grep_largest_blob(roi, bin);
  bitmap_complement(bin, roi);
  write_bitmap_to_screen(bin, bin, bin);
  wait_keyhit();
  ...
  */
  ...
  // up image statistic /////////////////////////
  bytemap_get_min_max_on_roi(&up_min, &up_max, up_image, roi);
  bytemap_get_mean_on_roi(&up_mean, up_image, roi);
  bytemap_get_variance_on_roi(&up_var,  up_image, roi);
  printf("Up image statistics: min(%d), mean(%lf), max(%d), variance(%lf)\n",
	 up_min, up_mean, up_max, up_var);
  ////////////////////////////////////////////////

  // mid image statistic /////////////////////////
  bytemap_get_min_max_on_roi(&mid_min, &mid_max, mid_image, roi);
  bytemap_get_mean_on_roi(&mid_mean, mid_image, roi);
  bytemap_get_variance_on_roi(&mid_var,, mid_image, roi);
  printf("Mid image statistics: min(%d), mean(%lf), max(%d), variance(%lf)\n",
	 mid_min, mid_mean, mid_max, mid_var);
  /////////////////////////////////////////////////

  // dn image statistic ///////////////////////////
  bytemap_get_min_max_on_roi(&dn_min, &dn_max, dn_image, roi);
  bytemap_get_mean_on_roi(&dn_mean, dn_image, roi);
  bytemap_get_variance_on_roi(&dn_var, dn_image,  roi);
  printf("Dn image statistics: min(%d), mean(%lf), max(%d), variance(%lf)\n",
	 dn_min, dn_mean, dn_max, dn_var);
  /////////////////////////////////////////////////
