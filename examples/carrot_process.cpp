...
#include <thresholding/backsym.h>
#include <thresholding/dsigma.h>
#include <thresholding/isodata.h>
#include <thresholding/otsu.h>
#include <thresholding/triangulate.h>
#include <labeling/labeling.h>
#include <edge/kirsch.h>
#include <edge/prewitt.h>
#include <edge/roberts.h>
#include <edge/robinson.h>
#include <edge/scharr.h>
#include <edge/sobel.h>
#include <drawing/draw_line.h>
...
int carrot_process(int argc, char *argv[])
{
  ...
  char str[256], fn[256], *ptr;
  FILE *f;
  real_t mr, mg, mb;

  w = WIDTH; h = HEIGHT;

  printf("screen initialize...\n");
  ...
  printf("Red channel Calyx\n");
  bitmap_clear(calyx);
  bytemap_threshold(bin, a, 0, 155);
  binary_opening(calyx, bin, se);
  write_bitmap_to_screen(calyx, calyx, calyx);
  wait_keyhit();
  
  printf("Green channel Calyx\n");
  bytemap_threshold(bin, b, 100, 170);
  binary_opening(tmp, bin, se);
  write_bitmap_to_screen(tmp, tmp, tmp);
  wait_keyhit();

  bitmap_and(calyx, tmp);

  printf("Blue channel Calyx\n");
  bytemap_threshold(bin, c, 0, 100);
  binary_opening(tmp, bin, se);
  write_bitmap_to_screen(tmp, tmp, tmp);
  wait_keyhit();
  ...
  for (i = 0; i < h; i++) {
    for (j = 0; j < w; j++) {
      if (bitmap_isset(calyx, j, i)) {
	bitmap_reset_value(roi, j, i);
      }
    }
  }
  write_bitmap_to_screen(roi, roi, roi);
  ...
  */
  ...
  labeling_grep_big_blob(bin, roi);
  //bitmap_not(roi);
  ...
  */
  ...
  // Isolate an object with dominent area
  printf("Isolate the biggest object\n");
  i = labeling_grep_big_blob(bin, roi);
  ...
  for (i = 0; i < h; i++) {
	...
	num++;
	mr += bytemap_get_value(a, j, i);
	mg += bytemap_get_value(b, j, i);
	mb += bytemap_get_value(c, j, i);
      }
    }
  }
  ...
  */
  ...
  printf("Chain coding:");
  chain = chaincode_new();
  chaincoding_on_bitmap(chain, roi);
  ...
  f = fopen("sampling_point.txt", "a+");
  fprintf(f, "%s:(x, y)", fn);
  bytemap_clear(a);
  for (link = plist->tail->next; link != plist->head; link = link->next) {
    p = (point_t *)link->object;
    fprintf(f, ", (%lf, %lf)", p->x, p->y);
  ...
  fourier = vector_new(vector_get_dimension(time), true);
  fd = vector_new(vector_get_dimension(time), true);

  dft(fourier, time);
  i = vector_get_dimension(time);

  f = fopen("original_fourier_descriptor.txt", "a+");
  fprintf(f, "%s:(x,y)", fn);
  for (i = 0; i < vector_get_dimension(fourier); i++) {
    fprintf(f, ", (%lf, %lf)", vector_get_value(fourier, i), ivector_get_value(fourier, i));
  }
  fclose(f);
  ...
  fprintf(f, "%s", fn);
  for (i = 0; i < vector_get_dimension(fd); i++) {
    cvector_read_value(&comp, fd, i);
    val = complex_get_abs(&comp);
    vector_put_value(val, fd, i);
    ivector_put_value(0, fd, i);
    fprintf(f, ", %lf", val);
  }
  fprintf(f, "\n");
  fclose(f);
  ...
  for (i = ymin; i < ymax; i++) {
    for (j = xmin; j < xmax; j++) {
      if (check_position_in_polygon_with_xy_profiling(j, i, plist))
  ...
  */
  ...
  convexhulling(convexhull, plist, QUICK_HULL);
  bitmap_clear(bin);
  ...
  minrect = polygon_new();
  convexhull_compute_min_area_rectangle(minrect, convexhull);
  bitmap_clear(bin);
