...
int ImageSubtract(int argc, char *argv[])
{
	...
	sprintf(dirname, "%s\\%snm-%snm between %s and %s", argv[1], argv[2], argv[3], argv[4], argv[5]);
	mkdir(dirname);
	...
	putimage(0, 0, display_buffer, COPY_PUT);
	getch();
	sprintf(filename, "%s\\result over %snm.bmp", dirname, argv[2]);
	...
	putimage(0, 0, display_buffer, COPY_PUT);
	getch();
	sprintf(filename, "%s\\result over %snm.bmp", dirname, argv[3]);
	writeimagefile(filename);
