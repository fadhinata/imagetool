...
// argv[4] : cutup threshold value
// argv[5] : cutdown threshold value
int ImageDivide(int argc, char *argv[])
{
	...
	initwindow(display_width, display_height, "ImageDivide");
	...
	for (i = 0; i < display_height; i++) {
		for (j = 0; j < display_width; j++) {
			if (*(roi->buffer+i*(roi->header.pitch)+j) != 0) {
				value = (int)(1000.0*(double)(*(op1->buffer+i*(op1->header.pitch)+j))/(double)(*(op2->buffer+i*(op2->header.pitch)+j)));
	...
	sprintf(dirname, "%s\\%snm over %snm between %s and %s", argv[1], argv[2], argv[3], argv[4], argv[5]);
	mkdir(dirname);
