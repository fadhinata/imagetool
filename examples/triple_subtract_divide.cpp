 ...
 * argv[3] : the number of op1
 * argv[4] : the number of op2
 * argv[5] : the number of a numerator
 * argv[6] : cutup threshold
 * argv[7] : cutdown threshold
 */
int TripleSubtractDivide(int argc, char *argv[])
{
	...
	unsigned char *abuf, *cbuf, *dbuf;
	bytemap_t *bytemap, *se;
	bitmap_t *bin;
	...
	int op1, op2, numerator;
	
	op1 = atoi(argv[3]);
	sprintf(filename, "%s\\%sCam%d.bmp", argv[1], argv[2], op1);
	LoadImage(gray_images[0], filename);
	
	op2 = atoi(argv[4]);
	sprintf(filename, "%s\\%sCam%d.bmp", argv[1], argv[2], op2);
	LoadImage(gray_images[1], filename);
	
	numerator = atoi(argv[5]);
	sprintf(filename, "%s\\%sCam%d.bmp", argv[1], argv[2], numerator);
	LoadImage(gray_images[2], filename);
	...
	for (i = 0; i < display_height; i++) {
		for (j = 0; j < display_width; j++) {
			if ((*(roibuf+(j>>3)))&(1<<(j%8))) {
				m = (int)(1000.0*((double)(*(abuf+j))-(double)(*(bbuf+j)))/((double)(*(cbuf+j))+1.0));
				//printf("%d ", m);
		...
		dbuf += bytemap->header.pitch;
		roibuf += roi_image->header.pitch;
		binbuf += bin->header.pitch;
	}
	
	display_write_gray_image(display_buffer, 0, 0, gray_images[1]);
	...
	//display_write_red_image(display_buffer, 0, 0, bytemap);
	putimage(0, 0, display_buffer, COPY_PUT);
	sprintf(dirname, "%s\\result", argv[1]);
	mkdir(dirname);
	sprintf(filename, "%s\\%s_%dSub%dDiv%d_%d_%d.bmp", dirname, argv[2], op1, op2, numerator, cutup, cutdown);
