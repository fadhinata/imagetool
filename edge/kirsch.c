...
#define KIRSCH_GRADIENT(type, gx_buf, gx_w, gx_h, gx_pitch, gy_buf, gy_w, gy_h, gy_pitch, p_buf, p_w, p_h, p_pitch) { \
    ...
    for (y = 1; y < h-1; y++) {						\
      for (x = 1; x < w-1; x++) {					\
	*(gxbuf+x) = -5*(*(pbuf-ppitch+(x-1)))+3*(*(pbuf-ppitch+x))+3*(*(pbuf-ppitch+(x+1))) \
          -5*(*(pbuf+(x-1)))                            +3*(*(pbuf+(x+1))) \
          -5*(*(pbuf+ppitch+(x-1)))+3*(*(pbuf+ppitch+x))+3*(*(pbuf+ppitch+(x+1))); \
	*(gybuf+x) = -5*(*(pbuf-ppitch+(x-1)))-5*(*(pbuf-ppitch+x))-5*(*(pbuf-ppitch+(x+1))) \
          +3*(*(pbuf+(x-1)))                            +3*(*(pbuf+(x+1))) \
          +3*(*(pbuf+ppitch+(x-1)))+3*(*(pbuf+ppitch+x))+3*(*(pbuf+ppitch+(x+1))); \
      }									\
      pbuf += ppitch;							\
      gxbuf += gxpitch;							\
      gybuf += gypitch;							\
    ...
    gybuf = (type *)(gy_buf);						\
    for (x = 1; x < w-1; x++) {						\
      *(gxbuf+x) = -5*(*(pbuf+(x-1)))                            +3*(*(pbuf+(x+1))) \
        -5*(*(pbuf+ppitch+(x-1)))+3*(*(pbuf+ppitch+x))+3*(*(pbuf+ppitch+(x+1))); \
      *(gybuf+x) = +3*(*(pbuf+(x-1)))                            +3*(*(pbuf+(x+1))) \
        +3*(*(pbuf+ppitch+(x-1)))+3*(*(pbuf+ppitch+x))+3*(*(pbuf+ppitch+(x+1))); \
    }									\
                                                                        \
    ...
    gybuf = (type *)((gy_buf) + gypitch);                               \
    for (y = 1; y < h-1; y++) {						\
      *gxbuf = +3*(*(pbuf-ppitch))+3*(*(pbuf-ppitch+1))			\
        +3*(*(pbuf+1))							\
	+3*(*(pbuf+ppitch))+3*(*(pbuf+ppitch+1));			\
      *gybuf = -5*(*(pbuf-ppitch))-5*(*(pbuf-ppitch+1))			\
	+3*(*(pbuf+1))							\
	+3*(*(pbuf+ppitch))+3*(*(pbuf+ppitch+1));			\
      pbuf += ppitch;							\
      gxbuf += gxpitch;							\
    ...
    for (y = 1; y < h-1; y++) {						\
      *gxbuf = -5*(*(pbuf-ppitch-1))+3*(*(pbuf-ppitch))			\
	-5*(*(pbuf-1))							\
	-5*(*(pbuf+ppitch-1))+3*(*(pbuf+ppitch));			\
      *gybuf = -5*(*(pbuf-ppitch-1))-5*(*(pbuf-ppitch))			\
	+3*(*(pbuf-1))							\
	+3*(*(pbuf+ppitch-1))+3*(*(pbuf+ppitch));			\
      pbuf += ppitch;							\
      gxbuf += gxpitch;							\
      gybuf += gypitch;							\
    }									\
									\
    *((gx_buf)) =                         +3*(*((p_buf)+1))		\
      +3*(*((p_buf)+ppitch))+3*(*((p_buf)+ppitch+1));			\
    *((gy_buf)) =                         +3*(*((p_buf)+1))		\
      +3*(*((p_buf)+ppitch))+3*(*((p_buf)+ppitch+1));			\
									\
    *((gx_buf)+(h-1)*(gxpitch))						\
      = +3*(*((p_buf)+(h-2)*ppitch))+3*(*((p_buf)+(h-2)*ppitch+1))	\
      +3*(*((p_buf)+(h-1)*ppitch+1));					\
    *((gy_buf)+(h-1)*(gypitch))						\
      = -5*(*((p_buf)+(h-2)*ppitch))-5*(*((p_buf)+(h-2)*ppitch+1))	\
      +3*(*((p_buf)+(h-1)*ppitch+1));					\
									\
    *((gx_buf)+(w-1)) = -5*(*((p_buf)+(w-2)))				\
      -5*(*((p_buf)+ppitch+(w-2)))+3*(*((p_buf)+ppitch+(w-1)));		\
    *((gy_buf)+(w-1)) = +3*(*((p_buf)+(w-2)))				\
      +3*(*((p_buf)+ppitch+(w-2)))+3*(*((p_buf)+ppitch+(w-1)));		\
									\
    *((gx_buf)+(h-1)*(gxpitch)+(w-1))					\
      = -5*(*((p_buf)+(h-2)*ppitch+(w-2)))+3*(*((p_buf)+(h-2)*ppitch+(w-1))) \
      -5*(*((p_buf)+(h-1)*ppitch+(w-2)));				\
    *((gy_buf)+(h-1)*(gypitch)+(w-1))					\
      = -5*(*((p_buf)+(h-2)*ppitch+(w-2)))-5*(*((p_buf)+(h-2)*ppitch+(w-1))) \
      +3*(*((p_buf)+(h-1)*ppitch+(w-2)));				\
  }

void matrix_kirsch_gradient(matrix_t *grad, bytemap_t *p)
{
  KIRSCH_GRADIENT(real_t,
