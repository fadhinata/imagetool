...
#define ROBINSON_GRADIENT(type, gx_buf, gx_w, gx_h, gx_pitch, gy_buf, gy_w, gy_h, gy_pitch, p_buf, p_w, p_h, p_pitch) { \
    ...
    for (y = 1; y < h-1; y++) {						\
      for (x = 1; x < w-1; x++) {					\
	*(gxbuf+x) = -(*(pbuf-ppitch+(x-1)))+(*(pbuf-ppitch+x))+(*(pbuf-ppitch+(x+1))) \
	  -(*(pbuf+(x-1)))       -2*(*(pbuf+x))     +(*(pbuf+(x+1)))	\
	...
	*(gybuf+x) = -(*(pbuf-ppitch+(x-1)))-(*(pbuf-ppitch+x))-(*(pbuf-ppitch+(x+1))) \
    ...
    gybuf = (type *)(gy_buf);						\
    for (x = 1; x < w-1; x++) {						\
      *(gxbuf+x) = -(*(pbuf+(x-1)))       -2*(*(pbuf+x))     +(*(pbuf+(x+1))) \
	-(*(pbuf+ppitch+(x-1)))+(*(pbuf+ppitch+x))+(*(pbuf+ppitch+(x+1))); \
      *(gybuf+x) = +(*(pbuf+(x-1)))       -2*(*(pbuf+x))       +(*(pbuf+(x+1))) \
	+(*(pbuf+ppitch+(x-1)))+(*(pbuf+ppitch+x))+(*(pbuf+ppitch+(x+1))); \
    }									\
									\
    pbuf = (p_buf)+(h-1)*ppitch;					\
    gxbuf = (type *)((gx_buf)+(h-1)*(gxpitch));				\
    gybuf = (type *)((gy_buf)+(h-1)*(gxpitch));				\
    for (x = 1; x < w-1; x++) {						\
      *(gxbuf+x) = -(*(pbuf-ppitch+(x-1)))+(*(pbuf-ppitch+x))+(*(pbuf-ppitch+(x+1))) \
	-(*(pbuf+(x-1)))       -2*(*(pbuf+x))     +(*(pbuf+(x+1)));	\
      *(gybuf+x) = -(*(pbuf-ppitch+(x-1)))-(*(pbuf-ppitch+x))-(*(pbuf-ppitch+(x+1))) \
    ...
    pbuf = (p_buf)+ppitch;						\
    gxbuf = (type *)((gx_buf)+gxpitch);					\
    gybuf = (type *)((gy_buf)+gxpitch);					\
    for (y = 1; y < h-1; y++) {						\
      *gxbuf = +(*(pbuf-ppitch))+(*(pbuf-ppitch+1))			\
	-2*(*(pbuf))     +(*(pbuf+1))					\
    ...
    gybuf = (type *)((gy_buf)+gxpitch+(w-1));				\
    for (y = 1; y < h-1; y++) {						\
      *gxbuf = -(*(pbuf-ppitch-1))+(*(pbuf-ppitch))			\
	-(*(pbuf-1))       -2*(*(pbuf))					\
	...
	+(*(pbuf+ppitch-1))+(*(pbuf+ppitch));				\
      ...
      gybuf += gypitch;							\
									...
									\
    *((gx_buf)) = -2*(*((p_buf)))     +(*((p_buf)+1))			\
      +(*((p_buf)+ppitch))+(*((p_buf)+ppitch+1));			\
    *((gy_buf)) = -2*(*((p_buf)))     +(*((p_buf)+1))			\
      +(*((p_buf)+ppitch))+(*((p_buf)+ppitch+1));			\
									\
    *((gx_buf)+(h-1)*(gxpitch))						\
      = +(*((p_buf)+(h-2)*ppitch))  +(*((p_buf)+(h-2)*ppitch+1))	\
      -2*(*((p_buf)+(h-1)*ppitch))+(*((p_buf)+(h-1)*ppitch+1));		\
    *((gy_buf)+(h-1)*(gypitch))						\
      = -(*((p_buf)+(h-2)*ppitch))  -(*((p_buf)+(h-2)*ppitch+1))	\
      -2*(*((p_buf)+(h-1)*ppitch))+(*((p_buf)+(h-1)*ppitch+1));		\
									\
    *((gx_buf)+(w-1)) = -(*((p_buf)+(w-2)))       -2*(*((p_buf)+(w-1))) \
      -(*((p_buf)+ppitch+(w-2)))+(*((p_buf)+ppitch+(w-1)));		\
    *((gy_buf)+(w-1)) = +(*((p_buf)+(w-2)))       -2*(*((p_buf)+(w-1))) \
      +(*((p_buf)+ppitch+(w-2)))+(*((p_buf)+ppitch+(w-1)));		\
									\
    *((gx_buf)+(h-1)*(gxpitch)+(w-1))					\
      = -(*((p_buf)+(h-2)*ppitch+(w-2)))+(*((p_buf)+(h-2)*ppitch+(w-1))) \
      -(*((p_buf)+(h-1)*ppitch+(w-2)))-2*(*((p_buf)+(h-1)*ppitch+(w-1))); \
    *((gy_buf)+(h-1)*(gypitch)+(w-1))					\
      = -(*((p_buf)+(h-2)*ppitch+(w-2)))-(*((p_buf)+(h-2)*ppitch+(w-1))) \
      +(*((p_buf)+(h-1)*ppitch+(w-2)))-2*(*((p_buf)+(h-1)*ppitch+(w-1))); \
  }

void matrix_robinson_gradient(matrix_t *grad, bytemap_t *p)
{
  ROBINSON_GRADIENT(real_t,
...
void robinson_gradient(wordmap_t *gx, wordmap_t *gy, bytemap_t *p)
{
  ROBINSON_GRADIENT(int16_t,
		    wordmap_get_buffer(gx),
		    wordmap_get_width(gx),
		    wordmap_get_height(gx),
		    wordmap_get_pitch(gx) / sizeof(uint16_t),
		    wordmap_get_buffer(gy),
		    wordmap_get_width(gy),
		    wordmap_get_height(gy),
		    wordmap_get_pitch(gy) / sizeof(uint16_t),
		    bytemap_get_buffer(p),
		    bytemap_get_width(p),
		    bytemap_get_height(p),
		    bytemap_get_pitch(p) / sizeof(*(bytemap_get_buffer(p))));
...
void robinson_edge(bitmap_t *q, bytemap_t *p, real_t tlevel)
{
  int c, r, w, h;
  wordmap_t *gx, *gy;
  int16_t *gxbuf, *gybuf;
